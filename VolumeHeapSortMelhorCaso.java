...
public class VolumeHeapSortMelhorCaso {
    
    public static void main(String[] args) {
        String csvFile = "C:\\Users\\rodri\\Desktop\\Estudos\\Projeto LEDA\\b3stocks_ticker_heapSortFloat_medioCaso.csv"; // caminho para o arquivo CSV
        int columnToSort = 6; // índice da coluna a ser ordenada (começando de 0)
        
        try {
            String[] data = readColumn(csvFile, columnToSort);
            heapSort(data);
            writeCSV("b3stocks_ticker_heapSortFloat_melhorCaso.csv", data);
            System.out.println("Valores ordenados foram escritos no arquivo 'b3stocks_volume_heapSortFloat_melhorCaso.csv'.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
