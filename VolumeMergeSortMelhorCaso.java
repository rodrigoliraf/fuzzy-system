...
public class VolumeMergeSortMelhorCaso {
    
    public static void main(String[] args) {
        String csvFile = "C:\\Users\\rodri\\Desktop\\Estudos\\Projeto LEDA\\b3stocks_ticker_mergeSort_medioCaso.csv"; // caminho para o arquivo CSV
        int columnToSort = 1; // índice da coluna a ser ordenada (começando de 0)
        
        try {
            String[] data = readColumn(csvFile, columnToSort);
            mergeSort(data, 0, data.length - 1);
            writeCSV("b3stocks_ticker_mergeSort_melhorCaso.csv", data);
            System.out.println("Valores ordenados foram escritos no arquivo 'b3stocks_ticker_mergeSort_melhorCaso.csv'.");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
